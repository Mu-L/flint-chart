...
每个预设在注册表中有默认列表和支持列表；解析器拒绝不支持的手势，以及为不保留状态的预设设置的任何 `reset`。每张图表一个分派器运行这些手势。一个手势只重置列表中包含它的交互，各按自己的 id：清除保留的更新，有状态刷选通过其控制器清除，`navigate` 与 `brush-zoom` 经导航控制器飞回，带闭包状态的预设通过 `onReset()` 丢弃状态。飞回与刷选放大默认以 400 ms 动画完成；`resetTransition: { duration: 0 }` 则直接跳转。平移与滚轮缩放跟随指针，从不做动画。
